...
public class Model implements IModel {
  ...
  @Override
  public List<IKeyFrame> discreteKeyFrames(IShape s) {
    List<IKeyFrame> result = new ArrayList<>();

    for (IMotion m : this.getAllMotions(s.getID())) {
      for (int i = m.getStartTick(); i < m.getEndTick(); i ++) {
        IKeyFrame current = m.getKeyFrameAt(m.getStartTick());
        IShape newShape = current.getShape();
        IKeyFrame newKeyFrame = new KeyFrame(i,newShape);
        result.add(newKeyFrame);
      }
    }

    IKeyFrame newKeyFrame = this.getAllKeyFrames(s.getID())
            .get(this.getAllKeyFrames(s.getID()).size() - 1);
    result.add(newKeyFrame);
    return result;
  }
