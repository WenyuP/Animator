...
public class Motion implements IMotion {
  ...
  @Override
  public IMotion createHorizontalMotionForPlusShape(IMotion m) {
    IKeyFrame start = m.getKeyFrameAt(startTick);
    IKeyFrame end = m.getKeyFrameAt(endTick);

    List<IKeyFrame> newKeyFrames = new ArrayList<>();

    for (int i = 0; i < m.getKeyFrames().size(); i ++) {
      IShape newShape = m.getKeyFrames().get(i).getShape().getHorizontalRectangle();
      IKeyFrame newKeyFrame = new KeyFrame(m.getKeyFrames().get(i).getTick(),newShape);
      newKeyFrames.add(newKeyFrame);
    }


    IMotion newMotion = new Motion(m.getKeyFrames().get(0)
            .getShape().getHorizontalRectangle().getID()
            ,m.getStartTick(),m.getEndTick(),newKeyFrames);
    return newMotion;
  }

  @Override
  public IMotion createVerticalMotionForPlusShape(IMotion m) {
    IKeyFrame start = m.getKeyFrameAt(startTick);
    IKeyFrame end = m.getKeyFrameAt(endTick);
    ...
    for (int i = 0; i < m.getKeyFrames().size(); i ++) {
      IShape newShape = m.getKeyFrames().get(i).getShape().getVerticalRectangle();
      IKeyFrame newKeyFrame = new KeyFrame(m.getKeyFrames().get(i).getTick(),newShape);
      newKeyFrames.add(newKeyFrame);
    }

    IMotion newMotion = new Motion(m.getKeyFrames().get(0).getShape().getVerticalRectangle().getID()
            ,m.getStartTick(),m.getEndTick(),newKeyFrames);
    return newMotion;
  }
